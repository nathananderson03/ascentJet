...
angular.module('ascentApp')
    .controller('EditAccountController', function($scope, $http, $location, $cookies, $window, $routeParams, $compile, $filter, cookieStore, Account, Country, PasswordUpdate, UpdateEmail) {
        $scope.account = {}
        $scope.saved = false;
        ...
        $scope.onSelect = function() {
            if(angular.element(document).find('.ui-select-container').hasClass('open') == true){
        ...
        $scope.additionalForm = false;

        $scope.emailFormOpen = false;
        $scope.passFormOpen = false;

        $scope.cancel = function(param, form) {
            var formname = param;
            form.$setPristine(true);
            if(formname == 'password'){
                $scope.accountCredential.password = null;
                $scope.fake.repeat_password = null;
                $scope.passFormOpen = false;
                $('.passform-showhide').slideToggle();
                $('.passform-showhide input[type="password"]').attr('equals', '');
                $('.passform-showhide input[type="password"]').removeClass('ng-invalid ng-invalid-required ng-invalid-equals');
                if($scope.passMatch == true)
                    $scope.passMatch = false
            }else {
                $scope.accountCredential.email = null;
                $scope.confirm.email = null;
                $scope.emailFormOpen = false;
                $('.emailform-showhide').slideToggle();
                $('.emailform-showhide input[type="email"]').attr('equals', '');
                $('.emailform-showhide input[type="email"]').removeClass('ng-invalid ng-invalid-required ng-invalid-equals');
                if($scope.emailMatch == true)
                    $scope.emailMatch = false
                $('#emailUpdateForm').removeClass('ng-submitted');
            }

            form.$setPristine(true);
        }
        if(cookieStore.get('logged-in') == 'true')
            $scope.additionalForm = true;

        $scope.passUpdateShow = function() {
            if($scope.passFormOpen == false) {
                $('.passform-showhide').slideToggle();
                $scope.passFormOpen = true;
            }
        }
        $scope.updatePassword = function(isValid) {
            ...
            if(isValid){
                $scope.request = "";
                $scope.request= {
                    "password": $scope.accountCredential.password
                };

                PasswordUpdate.change($scope.request, function (data) {
                    $scope.response = data;
                    $scope.passwordUpdated = true;
                });
            } else {
                if ($scope.accountCredential.password == $scope.fake.repeat_password)
                    $scope.passMatch = false;
                else
                    $scope.passMatch = true;
            }
        }
        $scope.emailUpdateShow = function() {
            if($scope.emailFormOpen == false) {
                $('.emailform-showhide').slideToggle();
                $scope.emailFormOpen = true;
            }
        }
        $scope.updateEmail = function(isValid, form) {
            $scope.emailMatch = false;
            $('#emailUpdateForm').addClass('ng-submitted');
            if(isValid){
                $scope.request = "";
                $scope.request= {
                    "userName": cookieStore.get('userMailAddr'),
                    "newEmail": $scope.accountCredential.email
                };

                UpdateEmail.change($scope.request, function (data) {
                    $scope.response = data;
                    $scope.emailUpdated = true;
                });
            } else {
                if ($scope.accountCredential.email == $scope.confirm.email)
                    $scope.emailMatch = false;
                else
    ...
    .controller('LoginController', function($scope, $http, $log, $rootScope, $cookies, $cookieStore, $window, $routeParams, $compile, $filter, Login, Account, cookieStore, Logout) {
        ...
        $scope.$watch(function() {
            ...
            if ($cookieStore.get('logged-in') == true) {
                ...
                if(location.pathname != '/') {
                    $('div.toolbar a.login').closest('li').next().hide();
                }
            }
            else {
                ...
                Login.login($scope.login, function(data, headers) {
                    if (data.userId != null && headers('SESSIONID') != '') {
                        ...
                        Account.get({
                            ...
                            cookieStore.put('userMailAddr', $scope.user.email, {
                                path: "/"
                            });
        ...
        $scope.logout = function() {
            Logout.logout({}, function(data, headers) {
                ...
                cookieStore.remove('userMailAddr', false, {
                    path: "/"
                });
                $window.location.href = '/';
            });
...
angular.module('ascentApp')
    .controller('PaidController', function($scope, $http, $cookies, $window, $routeParams, $compile, $filter, cookieStore, Passanger, OfferSubmit) {
        $scope.offer = {};
        $scope.offer.id = $routeParams.id;
        $scope.userMailAddr = cookieStore.get('userMailAddr');
        $scope.userName = cookieStore.get('userMail')
        ...
        OfferSubmit.save({
            ...
            $http({
                method: 'GET',
                url: '/flights/payment_success',
                params: {
                    id: $routeParams.id,
                    data: data,
                    userEmail: $scope.userMailAddr,
...
angular.module('ascentApp')
    .controller('PasswordController', function($scope, $http, $cookies, $window, $routeParams, $compile, $filter, Password, cookieStore) {
        $scope.account = {}
        $scope.confirm = {}
        $scope.send = function(isValid) {
            if (isValid) {
                cookieStore.put('userMailAddr', $scope.account.email, {
                    path: "/"
                });
                Password.save($scope.account, function(data) {
                    $scope.response = data;
                    $window.location.href = '/account/#/password-reset-notice';
                });
            }
            if($scope.account.email == $scope.confirm.email)
                $scope.isMatch = false;
            else
                $scope.isMatch = true;
        }
...
angular.module('ascentApp')
    .controller('passwordResetController', function($scope, $http, $cookies, $window, $routeParams, $compile, $filter, cookieStore, Password, PasswordReset) {
        $scope.account = {};
        $scope.fake = {};
        $scope.oldPassword = $routeParams.oldPassword;
        $scope.request = "";
        $scope.send = function(isValid) {
            if(isValid) {
                if(cookieStore.get('userMailAddr')) {
                    $scope.request= {
                        "email": cookieStore.get('userMailAddr'),
                        "oldPassword": $scope.oldPassword,
                        "newPassword": $scope.account.password
                    };

                    PasswordReset.change($scope.request, function (data) {
                        $scope.response = data;
                        $window.location.href = '/account/#/password-changed';
                    });
                }
            }
            if($scope.account.password == $scope.fake.repeat_password)
                $scope.isMatch = false;
            else
                $scope.isMatch = true;
        }
...
angular.module('ascentApp')
    .controller('ContactController', function($scope, $http, $cookies, $window, $routeParams) {
        $scope.send = function(isValid) {
            /*if(isValid) {
                var temp = false;
            }*/
...
angular.module('ascentApp')
    .controller('SearchController', function($rootScope, $scope, $http, $cookies, $cookieStore, cookieStore, $timeout, $window, $routeParams, $compile, $filter, $location, localStorageService, Empty, EmptyReserve, Quote, Airport, uiGmapGoogleMapApi, uiGmapIsReady, Country, myAppServices, ieVersion, $geolocation) {
        ...
        $scope.showDetails = function($event, obj) {
            ...
            if( $($event.target).html() == "Details" )
                AscentJet.initGallery($this.parents('tr').next().find('div.expander'));
        }
